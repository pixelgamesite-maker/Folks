...
export default function EarlyRoleModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function verifyTweet() {
    ...
    const { data, error } = await supabase.functions.invoke("verify-post", {
      body: { url: tweetUrl, mode: "mention" },
    });
    setVerifyingTweet(false);
    if (error || !data?.verified) {
      setTweetError(data?.reason || "Couldn't verify that post. Try again.");
      return;
    }
    setPosted(true);
  }
              ...
              {!c2 ? (
                <>
                  ...
                  {isValidTweetUrl(tweetUrl) && (
                    <button onClick={verifyTweet} disabled={verifyingTweet} style={{ ...confirmBtn, cursor: verifyingTweet ? "wait" : "pointer" }}>
                      {verifyingTweet ? "Verifying..." : "Confirm Post"}
                    </button>
                  )}
                  {tweetError && <p style={{ fontFamily: body, fontSize: "0.6rem", color: "#d96b5a", margin: "6px 0 0" }}>{tweetError}</p>}
                </>
              ) : (
